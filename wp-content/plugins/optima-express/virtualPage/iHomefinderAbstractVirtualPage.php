...
abstract class iHomefinderAbstractVirtualPage implements iHomefinderVirtualPageInterface {
	...
	protected function getKestrelBody($path) {
		$authenticationToken = $authenticationToken = iHomefinderAdmin::getInstance()->getAuthenticationToken();
		$sessionId = $this->stateManager->getSessionId();
		$leadCaptureUserId = $this->stateManager->getLeadCaptureUserId();
		return "
			<script>
				document.currentScript.replaceWith(ihfRenderKestrel({
					authenticationToken: \"" . $authenticationToken . "\",
					platform: \"oe\",
					sessionId: \"" . $sessionId . "\",
					leadCaptureUserId: " . ($leadCaptureUserId ? $leadCaptureUserId : 0) . ",
					path: \"" . $path . "\",
				}));
			</script>
		";
