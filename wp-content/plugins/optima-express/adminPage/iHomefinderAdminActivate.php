			...
			if(count($errors) === 0) {
				...
				<script type="text/javascript">
					jQuery(document).ready(function() {
						jQuery('#accountType').on('change', function() {
							var inputValue = jQuery(this).attr("value");
							if (inputValue == 'broker') {
								jQuery("#agentCrmProductType").hide();
							} else {
								jQuery("#agentCrmProductType").show();
							}
						});
					});
				</script>
						...
						<tr id="agentCrmProductType">
							...
							<td>
								<select id="agentCrm" name="agentCrm">
									<option>Select One</option>
									<option value="false">Premium (no CRM)</option>
									<option value="true">Premium + CRM</option>
								</select>
							</td>
						</tr>
					</table>
		...
		<?php } else { ?>
			<?php if(!$this->admin->isActivated()) { ?>
				...
				<p>
					...
					<a href="https://www.ihomefinder.com/products/optima/optima-express-agent-pricing/?plugin=true&redirectURL=<?php echo urlencode(admin_url("admin.php?page=" . iHomefinderConstants::PAGE_ACTIVATE)); ?>" class="button button-primary ihf-button-large">Sign Up for a<br />Paid Account</a>
				</p>
